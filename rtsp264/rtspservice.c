...
unsigned char sps_tmp[256];
unsigned char pps_tmp[256];
int sps_len=0;
int pps_len=0;
...
*/
	...
	strcat(pDescr, "\r\n");
	strcat(pDescr, "s=H.264 Video, streamed by the Zyqhziee Media Server\r\n");
	...
	strcat(pDescr, "a=x-qt-text-nam:H.264 Video, streamed by the Zyqhziee Media Server\r\n");	
	strcat(pDescr, "a=x-qt-text-inf:test.h264\r\n");	
...
void UpdateSps(unsigned char *data,int len)
{
	...
	sprintf(psp.base64profileid,"%02X%02X%02X",data[1],data[2],data[3]);//sps[0] 0x67
	#if 0
