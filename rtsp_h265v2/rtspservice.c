...
struct profileid_vps_sps_pps{
	char base64profileid[10];
	char base64_level_idc[10];
	char base64vps[524];
	char base64sps[524];
	char base64pps[524];
	char base64sei[524];
...
struct profileid_vps_sps_pps psp; //存base64编码的profileid sps pps
...
unsigned char vps_tmp[512];
unsigned char sps_tmp[512];
unsigned char pps_tmp[512];
unsigned char sei_tmp[512];
...
extern unsigned long server_port;
...
void PrefsInit()
{
	...
	printf("\tInput rtsp://hostIP:%d/test.265 to play this\n",stPrefs.port);
	printf("\n");
#endif
...
*/
	...
	strcat(pDescr, "\r\n");
	strcat(pDescr, "s=H.265 Video, streamed by the Zyqhziee Media Server\r\n");
	strcat(pDescr, "i=test.h265\r\n");
   	strcat(pDescr, "t=0 0\r\n");	
   	strcat(pDescr, "a=tool:Zyqhziee Streaming Media v2015.11.27\r\n");	
	strcat(pDescr, "a=type:broadcast\r\n");
	strcat(pDescr, "a=control:*\r\n");
	strcat(pDescr, "a=range:npt=0-\r\n");
	strcat(pDescr, "a=x-qt-text-nam:H.265 Video, streamed by the Zyqhziee Media Server\r\n");	
	strcat(pDescr, "a=x-qt-text-inf:test.h265\r\n");	
	...
	strcat(pDescr, "0.0.0.0");//inet_ntoa(((struct sockaddr_in *)(&pRtsp->stClientAddr))->sin_addr));
	strcat(pDescr, "\r\n");	 
	/**** media specific ****/
...
//	//strcat(pDescr, "\r\n");
	strcat(pDescr,"b=AS:500\r\n");
		...
		strcat(pDescr, "\r\n");
		strcat(pDescr,"a=fmtp:96 profile-space=0;");
		strcat(pDescr,"profile-id=1;");
		strcat(pDescr,"tier-flag=0;");
		strcat(pDescr,"level-id=");
		strcat(pDescr,psp.base64_level_idc);
		strcat(pDescr,";interop-constraints=B00000000000;");
		strcat(pDescr,"sprop-vps=");		
		strcat(pDescr,psp.base64vps);
		strcat(pDescr,";sprop-sps=");
		strcat(pDescr,psp.base64sps);
		strcat(pDescr,";sprop-pps=");
		strcat(pDescr,psp.base64pps);
		strcat(pDescr, "\r\n");
		strcat(pDescr,"a=control:track1");
		strcat(pDescr, "\r\n");

printf("\n\n%s,%d===>psp.base64profileid=%s,psp.base64vps=%s,psp.base64sps=%s,psp.base64pps=%s\n\n",__FUNCTION__,__LINE__,psp.base64profileid,psp.base64vps,psp.base64sps,psp.base64pps);
...
//				sprintf(s8Str + strlen(s8Str), "RTP/AVP;multicast;ttl=%d;destination=%s;port=", (int)DEFAULT_TTL, descr->multicast);
			...
			{
				uc_ip = ((struct sockaddr_in *)(&pRtsp->stClientAddr))->sin_addr.s_addr;
				sprintf(s8Str + strlen(s8Str), "RTP/AVP;unicast;client_port=%d-%d;destination=%d.%d.%d.%d;source=%d.%d.%d.%d;server_port=", \
						pRtpSes->transport.u.udp.cli_ports.RTP, \
						pRtpSes->transport.u.udp.cli_ports.RTCP,\
						(uc_ip&0xff),((uc_ip>>8)&0xff),((uc_ip>>16)&0xff),((uc_ip>>24)&0xff),\
						((local_ip>>24)&0xff),((local_ip>>16)&0xff),((local_ip>>8)&0xff),(local_ip&0xff));
			}

			sprintf(s8Str + strlen(s8Str), "%d-%d"RTSP_EL, server_port, server_port+1);
			break;
...
**
**************************************************************************************************/
void UpdateVps(unsigned char *data,int len)
{
	int i;
	if(len>29)
		return ;
	
	memcpy(vps_tmp,data,len);
	vps_len = len;
	...
	#else
	sprintf(psp.base64_level_idc,"%d",data[20]);//4/*data[0]=0x40 data[1]=0x01 ......  data[20] = 0x7b*/
	//a=fmtp:96 packetization-mode=1;profile-level-id=ee69bc;sprop-parameter-sets=xu5pvGeDzEef,qFT3dm==;
	base64_encode2(data, len, psp.base64vps, 512);//
	#endif
...
void UpdateSps(unsigned char *data,int len)
{
	...
	memcpy(sps_tmp,data,len);
	sps_len = len;

	//sprintf(psp.base64sps,"%d",data[2]);//data[1] data[2] data[3] for h264 :sps[0] 0x67 ,for h265:0x42 ,.. ,..
	#if 0
	///a=fmtp:96 packetization-mode=1;profile-level-id=4b9096;sprop-parameter-sets=8kuQlme8xyAX,+sIKBQ==;
	base64_encode((unsigned char *)data, psp.base64sps, len);///sps
	#else
	//a=fmtp:96 packetization-mode=1;profile-level-id=ee69bc;sprop-parameter-sets=xu5pvGeDzEef,qFT3dm==;
	base64_encode2(data, len, psp.base64sps, 1024);
	#endif
...
void UpdatePps(unsigned char *data,int len)
{
	...
	if(len>12)
		return ;
	memcpy(pps_tmp,data,len);
	pps_len = len;
	//sprintf(psp.base64pps,"%02X",data[2]);
	#if 0
...
**
**************************************************************************************************/
void UpdateSei(unsigned char *data,int len)
{
	int i;
	if(len>14)
		return ;
	memcpy(sei_tmp,data,len);
	sei_len = len;
	//sprintf(psp.base64pps,"%02X",data[2]);
	#if 0
	base64_encode((unsigned char *)data, psp.base64pps, len);//pps
	...
	base64_encode2(data, len, psp.base64sei, 512);
	#endif
